...
//    These were the main source of 847ms Script Evaluation + 267ms Rendering TBT
...
//    — these were causing forced reflows (the "forced reflow" audit in Lighthouse)
// ✅ [JS size] Removed unused import of Script (was causing HMR factory error)
...
const Header = memo(() => {
  ...
  // ✅ FIX: Close menu + always restore body scroll on route change
  useEffect(() => {
    setIsMenuOpen(false);
  ...
  // ✅ FIX: Restore body scroll on unmount (handles edge cases)
  useEffect(() => {
    return () => {
      document.body.style.overflow = "";
      document.body.style.position = "";
  ...
  const toggleMobileMenu = useCallback(() => {
    setIsMenuOpen((prev) => {
      ...
      if (typeof document !== "undefined") {
        // ✅ FIX: Use position:fixed + top offset to prevent iOS scroll-jump
        if (next) {
  ...
  const handleMobileLinkClick = useCallback(
    (e: React.MouseEvent, href: string) => {
      e.preventDefault();
      // ✅ FIX: Restore scroll position before navigation
      const scrollY = document.body.style.top;
      document.body.style.overflow = "";
      document.body.style.position = "";
      document.body.style.top = "";
  ...
  return (
    <>
      <style jsx global>{`
        /* ============================================================
           HEADER ANIMATIONS
           ✅ FIX: ALL expensive animations are desktop-only.
           Mobile gets zero animation overhead — massive TBT reduction.
           ============================================================ */
        ...
        @media (min-width: 1024px) and (prefers-reduced-motion: no-preference) {
          ...
          /* Apple glass button — desktop only */
          .apple-glass-btn {
            position: relative;
            overflow: hidden;
        ...
        /* ✅ FIX: MOBILE — strip ALL expensive effects */
        @media (max-width: 1023px) {
          .apple-glass-btn {
            -webkit-tap-highlight-color: transparent;
            /* No will-change, no overflow:hidden pseudo-elements */
          }
          .apple-glass-btn::before,
          .apple-glass-btn::after,
          .liquid-overlay,
          ...
          /* ✅ FIX: Faster slide-down on mobile for perceived responsiveness */
        ...
        /* ✅ FIX: glass-border defined once, not per-render inline */
        .glass-border {
          border: 1px solid rgba(255, 255, 255, 0.2);
        }

        /* ✅ FIX: Logo content-visibility hint */
        .logo-img {
      ...
      <header
        ...
        style={{
          // ✅ FIX: On mobile, skip backdrop-blur entirely (massive paint cost)
          // Applied via inline style so it cascades over Tailwind without @media in JSX
      ...
      >
        {/*
          ✅ FIX: backdrop-blur only on desktop via a child div, not on the fixed header itself.
          This prevents the browser from creating a new compositing layer for the entire
          above-the-fold area on mobile, which was responsible for the ~267ms rendering TBT.
        */}
        <div
        ...
        <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-0">
            ...
            <nav className="hidden lg:flex items-center space-x-0" role="navigation" aria-label="Main menu">
              <Link
                href="/"
                className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap apple-glass-btn"
              ...
              >
                <button
                  className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap flex items-center space-x-1 apple-glass-btn"
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 transition-apple ${isCarRefinanceOpen ? "rotate-180" : ""}`}
                    aria-hidden="true"
                  />
                  <div className="liquid-overlay" />
                </button>
              ...
              <Link href="/car-loan" className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap apple-glass-btn" aria-current={pathname === "/car-loan" ? "page" : undefined}>
                <span className="relative z-10">New Car Loan</span>
                <div className="liquid-overlay" />
              </Link>
              <Link href="/personal-loan" className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap apple-glass-btn" aria-current={pathname === "/personal-loan" ? "page" : undefined}>
                <span className="relative z-10">Personal Loan</span>
                <div className="liquid-overlay" />
              </Link>
              <Link href="/property-loan" className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap apple-glass-btn" aria-current={pathname === "/property-loan" ? "page" : undefined}>
                <span className="relative z-10">Property Loan</span>
                <div className="liquid-overlay" />
              </Link>
              <Link href="/commercial-vehicle-loan" className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap apple-glass-btn" aria-current={pathname === "/commercial-vehicle-loan" ? "page" : undefined}>
                <span className="relative z-10">Commercial Vehicle</span>
                <div className="liquid-overlay" />
              </Link>
              <Link href="/contact" className="nav-link-water relative text-gray-700 hover:text-blue-600 transition-apple font-medium text-base whitespace-nowrap apple-glass-btn" aria-current={pathname === "/contact" ? "page" : undefined}>
              ...
              <Link
                href="/apply-now"
                className="apple-glass-btn relative bg-gradient-to-r from-blue-600 to-cyan-500 text-white px-5 py-2 rounded-full font-semibold hover:shadow-lg hover:shadow-blue-500/25 transition-apple text-base whitespace-nowrap glass-border ml-2"
                aria-label="Apply for a loan now"
              >
                <span className="relative z-10">Apply Now</span>
                <div className="liquid-overlay" />
              ...
              <Link
                href="/emi-calculator"
                className="apple-glass-btn relative bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 transition-apple font-medium text-base whitespace-nowrap flex items-center p-2.5 rounded-lg ml-1 overflow-hidden glass-border"
          ...
          {/* ✅ FIX: Mobile Menu — no backdrop-blur, no heavy animations */}
          {isMenuOpen && (
            <div
              id="mobile-menu"
              className="md:hidden py-4 bg-white rounded-2xl mt-2 border border-gray-100 mx-2 animate-slide-down shadow-xl"
              role="dialog"
              aria-modal="true"
            ...
            >
              <nav className="flex flex-col space-y-1 px-5" role="menu" aria-label="Mobile menu">
                ...
                <Link
                  ...
                  className="relative text-white bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-3 rounded-full font-semibold mt-3 text-center max-w-[220px] mx-auto glass-border text-base min-h-[44px] flex items-center justify-center"
                  role="menuitem"
                  aria-label="Apply for a loan now"
                >
                  <span className="relative z-10">Apply Now</span>
