...
export default function WhatsAppChatsManager({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return (
    <div className={embedded ? 'h-full flex flex-col gap-3' : 'space-y-4'}>
      {/* Header */}
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-4 ${embedded ? 'flex-1 min-h-0' : ''}`}>
        {/* ── Conversation list ─────────────────────────────── */}
        <div className={`lg:col-span-4 ${selected ? 'hidden lg:block' : ''} ${embedded ? 'h-full min-h-0' : ''}`}>
          <div className={`bg-white rounded-xl border border-gray-200 overflow-hidden ${embedded ? 'flex flex-col h-full' : ''}`}>
            <form onSubmit={onSearch} className="p-3 border-b border-gray-100 flex-shrink-0">
            ...
            <div className={`overflow-y-auto divide-y divide-gray-100 ${embedded ? 'flex-1 min-h-0' : 'max-h-[65vh]'}`}>
              {loadingList ? (
        ...
        <div className={`lg:col-span-8 ${selected ? '' : 'hidden lg:block'} ${embedded ? 'h-full min-h-0' : ''}`}>
          <div className={`bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col ${embedded ? 'h-full min-h-0' : 'min-h-[65vh]'}`}>
            {!selected ? (
              ...
              <>
                {/* Conversation header */}
                <div className="flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-cyan-50">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <button
                      onClick={() => { setSelected(null); setMessages([]); }}
                      className="lg:hidden p-1 rounded-lg hover:bg-white/70 flex-shrink-0"
                    ...
                    <div className="min-w-0">
                      ...
                      {selectedConvo && (
                        <div className="text-[11px] text-gray-500 flex items-center gap-1 truncate">
                          <Clock className="w-3 h-3 flex-shrink-0" /> <span className="truncate">{selectedConvo.count} messages · {timeAgo(selectedConvo.firstAt)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                    ...
                    <div className="inline-flex items-center rounded-lg border border-gray-200 bg-white p-0.5 shadow-sm">
                      <button
                        ...
                        disabled={modeSaving}
                        className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-md text-xs font-semibold transition ${
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 p-1.5 sm:px-2.5 sm:py-1.5 rounded-lg text-red-600 text-xs font-medium hover:bg-red-50 transition"
                ...
                {/* Messages */}
                <div ref={scrollRef} className={`flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4 bg-gray-50/50 ${embedded ? '' : 'max-h-[60vh]'}`}>
                  {loadingConvo ? (
                          ...
                          {m.botReply && (
                            <div className="flex justify-start">
                              <div className="max-w-[80%] relative group/bub">
                                ...
                                <div className="flex items-center flex-wrap gap-x-1.5 gap-y-0.5 mt-1 pl-1">
                                  {isAdmin ? (
                                    <>
                ...
                <div className="relative border-t border-gray-100 p-3 bg-white">
                  ...
                  <div className="flex items-end gap-1 sm:gap-1.5">
                    ...
                    <button
                      ...
                      className={`p-2 sm:p-2.5 rounded-xl border transition flex-shrink-0 ${
