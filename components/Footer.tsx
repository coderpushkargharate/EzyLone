...
// - Removed minHeight:600px → let content determine height (prevents forced layout)
// - Background decorative blurs completely removed on mobile (not just hidden)
// - All animations & pulse effects disabled on mobile
// - Simplified gradient backgrounds on mobile (single gradient vs multi-stop)
// - Reduced DOM nesting depth by merging containers
// - Removed heavy box-shadows and border effects on mobile
// - Logo: already lazy loaded with quality=60, preserved
// - All hover effects disabled on mobile (prevents repaints)
...
const Footer = memo(() => {
  ...
  return (
    <footer
      className="bg-slate-900 text-gray-100 relative overflow-hidden"
      aria-label="Site footer"
    >
      {/* 
        ✅ Mobile Optimized Backgrounds:
        - On mobile: simple single gradient (no via, no heavy transitions)
        - On desktop: full complex gradient with via + decorative blurs
      */}
      <div className="absolute inset-0 -z-10">
        {/* Mobile-optimized gradient (lightweight, single stop) */}
        <div className="absolute inset-0 bg-gradient-to-b from-slate-900 to-blue-950 lg:hidden" />
        
        {/* Desktop-only gradient + decorative blurs */}
        <div className="hidden lg:block">
          <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900" />
          
          {/* Background decorations - only visible on desktop */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-gradient-to-br from-cyan-500/10 to-blue-600/10 rounded-full blur-3xl animate-pulse" />
            <div className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-gradient-to-br from-blue-600/10 to-purple-600/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="lg:col-span-2 space-y-5">
              <div className="flex items-center space-x-3">
                <div className="relative shrink-0 w-12 h-12">
                  <div className="w-12 h-12 bg-gradient-to-br from-cyan-500 via-blue-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-cyan-500/30 border border-cyan-400/30">
                    <Image
              ...
              {/* Trust Badge - simpler on mobile */}
              <div className="bg-blue-900/50 rounded-xl p-4 border border-cyan-500/20">
              ...
              {/* Social Links - simplified hover effects only on desktop */}
              <div>
                  ...
                  {socialLinks.map((social, index) => {
                    ...
                    return (
                      <a
                        ...
                        className="w-10 h-10 bg-blue-800/40 rounded-xl flex items-center justify-center border border-cyan-500/30 transition-colors duration-200 lg:hover:bg-gradient-to-br lg:hover:from-cyan-500 lg:hover:to-blue-600"
                      >
              ...
              <ul className="space-y-2">
                {footerLinks.products.map((link, index) => (
                  <li key={index}>
                    ...
                    >
                      <span className="lg:group-hover:translate-x-0.5 transition-transform duration-200">{link.name}</span>
                      <ArrowRight className="w-3 h-3 ml-2 opacity-0 lg:group-hover:opacity-100 transition-all duration-200 text-cyan-400" />
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        {/* Contact + Compliance Section - simplified for mobile */}
        <div className="py-2 border-t border-cyan-500/20">
          <div className="bg-amber-900/30 rounded-2xl p-4 md:p-6 border border-amber-500/30">
            <div className="grid md:grid-cols-2 gap-6">
              {/* Contact Info */}
              <div className="space-y-4">
                <div className="text-center md:text-left">
                  <h3 className="text-lg font-semibold text-white mb-2 flex items-center justify-center md:justify-start">
                    <span className="w-3 h-3 bg-cyan-400 rounded-full mr-2" />
                    📞 Get in Touch
                  </h3>
                  <div className="w-16 h-0.5 bg-gradient-to-r from-cyan-400 to-blue-400 mx-auto md:mx-0 rounded-full" />
                </div>
                <div className="space-y-3">
                  ...
                  <a
                    ...
                    className="flex items-center space-x-3 p-3 bg-blue-900/50 rounded-lg border border-cyan-500/30"
                  >
                    <div className="w-9 h-9 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg flex items-center justify-center shadow-md shrink-0">
                  ...
                  <div className="flex items-start space-x-3 p-3 bg-blue-900/50 rounded-lg border border-cyan-500/30">
                    <div className="w-9 h-9 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg flex items-center justify-center mt-0.5 shadow-md shrink-0">
              ...
              {/* Legal Notice */}
              <div className="py-2">
                <div className="bg-slate-800/70 rounded-xl p-4 border border-amber-500/40">
        ...
        {/* Legal Links - simplified container */}
        <div className="py-6 md:py-8 border-t border-cyan-500/20">
          <div className="bg-slate-800/60 rounded-2xl p-4 md:p-6 border border-cyan-500/20">
            ...
            <div className="flex flex-wrap justify-center gap-2 md:gap-3 mb-5">
              {footerLinks.legal.map((link, index) => (
                <Link
                  key={index}
                  href={link.href}
                  className="text-gray-300/90 hover:text-cyan-300 transition-colors duration-300 text-sm font-medium px-3 md:px-4 py-2 rounded-lg hover:bg-cyan-500/10"
                >
                  {link.name}
                </Link>
              ))}
        ...
        {/* Copyright - simplified footer */}
        <div className="py-4 border-t border-cyan-500/20">
          <div className="bg-slate-900/90 rounded-2xl p-4 md:p-6 border border-cyan-500/30">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              <div className="text-center md:text-left">
                <div className="text-gray-200/95 text-sm font-medium">
                  © {currentYear} <span className="text-cyan-300 font-bold">EzyLoan</span>. All rights reserved.
                </div>
              </div>
              <div className="text-center md:text-right">
                <div className="text-gray-200/95 text-sm font-medium">
                  Designed with <span className="text-red-400">❤️</span> for your financial success
                </div>
            ...
            <div className="mt-6 pt-5 border-t border-cyan-500/20">
              <div className="flex flex-wrap justify-center items-center gap-4 md:gap-5 text-xs text-gray-400/90">
