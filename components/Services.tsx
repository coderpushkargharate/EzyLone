    ...
    <section id="services" className="py-10 bg-gradient-to-br from-slate-50 via-blue-50/50 to-cyan-50/50 relative overflow-hidden" aria-labelledby="services-heading" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      ...
      <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="hidden lg:block relative">
          ...
          <div className="flex justify-center gap-1 mt-8">{Array.from({ length: maxIndex + 1 }).map((_, index) => (<button key={index} onClick={() => goToSlide(index)} className="inline-flex h-6 min-w-[24px] items-center justify-center rounded-full px-1.5" aria-label={`Go to slide ${index + 1}`} aria-current={currentIndex === index ? "true" : "false"}><span className={`h-2.5 rounded-full transition-all duration-300 ${currentIndex === index ? "w-10 bg-gradient-to-r from-[#2563eb] to-[#06b6d4] shadow-md" : "w-2.5 bg-slate-300/60"}`} /></button>))}</div>
        </div>
