    ...
    <section id="services" className="py-10 bg-gradient-to-br from-slate-50 via-blue-50/50 to-cyan-50/50 relative overflow-hidden" aria-labelledby="services-heading" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      ...
      <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="hidden lg:block relative">
          <div className="overflow-hidden"><div className="flex transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${currentIndex * (100 / cardsPerView)}%)`, willChange: "transform" }}>{[...services, ...services.slice(0, cardsPerView)].map((service, index) => (<div key={`${service.title}-${index}`} className="flex-shrink-0 w-1/4 px-3" style={{ contain: "layout style paint" }}><Link href={service.link} className="block group"><div className="relative bg-white/60 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/50 hover:border-[#2563eb]/50 transition-all duration-300 flex flex-col h-full cursor-pointer shadow-md hover:shadow-lg hover:-translate-y-1"><div className="relative overflow-hidden bg-gradient-to-br from-gray-50/80 to-gray-100/80" style={{ aspectRatio: "16/9" }}><Image src={service.image} alt={service.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500 ease-out" sizes="(max-width: 1280px) 25vw, 300px" loading="lazy" quality={70} onError={(e) => { (e.target as HTMLImageElement).src = `https://placehold.co/400x225/f5f5f5/666?text=${encodeURIComponent(service.title)}`; }} /><div className="absolute inset-0 bg-gradient-to-t from-black/5 to-transparent pointer-events-none" /></div><div className="p-5 flex flex-col flex-grow"><h3 className="text-lg font-bold text-slate-800 mb-3 text-center group-hover:text-[#2563eb] transition-colors duration-300">{service.title}</h3><div className="space-y-1.5 mb-5">{service.highlights.map((highlight, i) => (<p key={i} className="text-sm text-slate-600/80 text-center font-medium">{highlight}</p>))}</div><div className="mt-auto"><button className="w-full inline-flex items-center justify-center gap-2 font-semibold text-sm py-3 px-4 rounded-xl transition-all duration-300 cursor-pointer shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 bg-gradient-to-r from-[#2563eb] to-[#06b6d4] text-white touch-manipulation" onClick={(e) => handleApplyClick(e, service.applyLink)} aria-label={`Get offers for ${service.title}`}>Get Offers <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" /></button></div></div></div></Link></div>))}</div></div>
        ...
        <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4">{services.map((service, index) => (<Link key={index} href={service.link} className="block"><div className="relative bg-white rounded-2xl overflow-hidden border border-gray-200 flex flex-col cursor-pointer shadow-sm"><div className="relative overflow-hidden bg-gray-100" style={{ aspectRatio: "16/9" }}><Image src={service.image} alt={service.title} fill className="object-cover" sizes="(max-width: 640px) calc(100vw - 32px), calc(50vw - 24px)" loading="lazy" quality={65} onError={(e) => { (e.target as HTMLImageElement).src = `https://placehold.co/400x225/f5f5f5/666?text=${encodeURIComponent(service.title)}`; }} /></div><div className="p-4 flex flex-col flex-grow"><h3 className="text-base font-bold text-slate-800 mb-2 text-center">{service.title}</h3><div className="space-y-1 mb-4">{service.highlights.map((highlight, i) => (<p key={i} className="text-xs text-slate-600 text-center">{highlight}</p>))}</div><button className="w-full inline-flex items-center justify-center gap-2 font-semibold text-sm py-2.5 px-3 rounded-xl bg-gradient-to-r from-[#2563eb] to-[#06b6d4] text-white touch-manipulation active:scale-95 transition-transform duration-100 min-h-[44px]" onClick={(e) => handleApplyClick(e, service.applyLink)} aria-label={`Get offers for ${service.title}`}>Get Offers <ArrowRight className="w-4 h-4" /></button></div></div></Link>))}</div>
      </div>
